...
默认开源镜像保持 C8 蜂窝 CPE 与 HomeProxy/sing-box 代理基线，并预装 TTYD/Web 终端、UPnP、WOL Ultra（已替代经典 WOL，路径 `admin/control/wolultra`）和 ZeroTier；不再预装 `luci-app-partexp` 和 LuCI 包管理页面。需要分区维护时，可通过 `Config/PRIVATE.txt` 或刷机后的命令行包管理器按需启用。
