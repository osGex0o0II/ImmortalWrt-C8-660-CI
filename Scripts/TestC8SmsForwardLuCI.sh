...
require_pattern "$VIEW" 'validateProxy' "proxy URL validator"
require_pattern "$VIEW" 'validateUrl' "HTTP URL validator"
require_pattern "$VIEW" 'printable\(32\)' "channel name validator"
...
require_pattern "$VIEW" 'm\.save\(\)' "actions save current form before backend calls"
